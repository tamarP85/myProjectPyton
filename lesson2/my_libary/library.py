from lesson2.my_libary.book import Book
...
class Library:
    ...
    def return_book(self, username, book):
        ...
        if book not in self.books:
            raise ValueError(f"Book '{book.title}' by {book.author} is not in the library.")
        if  self.checked_out_books[username] != book:
    ...
    def search_books(self, search_term):
        ...
        print([book for book in self.books if search_term.lower() in book.title.lower()][0].title)
